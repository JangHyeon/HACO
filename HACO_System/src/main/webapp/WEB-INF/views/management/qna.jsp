						...
						<div class="col-xs-12">
							...
							<ul class="pagination pagination-centered">
								...
								<li<c:if test="${beginpage<10}"> class="disabled"</c:if>>
									<a<c:if test="${beginpage>10}"> href="${pageContext.request.contextPath}/management/qna/pageSize/${pageSize}/pageNum/${beginpage-1}/searchType/${searchType}/searchKey/${searchKey}"</c:if>>«</a>
								</li>
								...
								<c:forEach var="i" begin="${beginpage}" end="${endpage}" step="1">
									...
									<c:if test="${i!=pageNum}">
										<li><a href="${pageContext.request.contextPath}/management/qna/pageSize/${pageSize}/pageNum/${i}/searchType/${searchType}/searchKey/${searchKey}">${i}</a></li>
									</c:if>
								</c:forEach>
								</c:if>
							  	<!-- 다음링크 -->
								<li<c:if test="${endpage>=pagecount}"> class="disabled"</c:if>>
									<a<c:if test="${endpage<pagecount}"> href="${pageContext.request.contextPath}/management/qna/pageSize/${pageSize}/pageNum/${endpage+1}/searchType/${searchType}/searchKey/${searchKey}"</c:if>>»</a>
								</li>
							</ul>
						</div>					
...
$(document).ready(function(){
	...
	var submit = function(){
		if($.trim($('input[name=searchKey]').val()).length==0){
			...
			location.href="${pageContext.request.contextPath}/management/qna/pageSize/"+$('input[name=pageSize]').val()+"/pageNum/1/searchType/"+$('input[name=searchType]').val()+"/searchKey/"+$.trim($('input[name=searchKey]').val());
		}
	}
	...
	$('#writeBtn').on('click', function() {
		location.href='${pageContext.request.contextPath}/management/questionWrite';
	});
