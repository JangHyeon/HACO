							...
							<div class="col-md-12 mt">
								<div class="content-panel">
										...
										<tbody id="topNotice">
											<c:forEach var="top" items="${topNoticeList}">
												<tr class="topNotice">
													...
													<td><a href="notice/${top.notice_id}">${top.title}</a></td>
														<s:authorize ifAnyGranted="TEACHER,MANAGER,CENTER,MASTER">
													<td>${top.name_kor}</td>
													</s:authorize>
									...
									<div class="col-xs-12">
										...
										<ul class="pagination pagination-centered">
											...
											<li<c:if test="${beginpage<10}"> class="disabled"</c:if>>
												<a<c:if test="${beginpage>10}"> href="${pageContext.request.contextPath}/notice?pageNum=${beginpage-1}&pageSize=${pageSize}&searchType=${searchType}&searchKey=${searchKey}"</c:if>>«</a>
											</li>
											...
											<c:forEach var="i" begin="${beginpage}" end="${endpage}" step="1">
												...
												<c:if test="${i!=pageNum}">
													<li><a href="${pageContext.request.contextPath}/notice?pageNum=${i}&pageSize=${pageSize}&searchType=${searchType}&searchKey=${searchKey}">${i}</a></li>
												</c:if>
											</c:forEach>
											</c:if>
										  	<!-- 다음링크 -->
											<li<c:if test="${endpage>=pagecount}"> class="disabled"</c:if>>
												<a<c:if test="${endpage<pagecount}"> href="${pageContext.request.contextPath}/notice?pageNum=${endpage+1}&pageSize=${pageSize}&searchType=${searchType}&searchKey=${searchKey}"</c:if>>»</a>
											</li>
										</ul>
									</div>						
									...
									<div class="col-sm-6 col-sm-offset-3" style="clear:both">
										<form id="noticeSearch" action="${pageContext.request.contextPath}/noticeSearch">
